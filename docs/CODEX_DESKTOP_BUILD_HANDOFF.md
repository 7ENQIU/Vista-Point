...
- Версия проекта: `0.1.3`. Ранее опубликованный web-prerelease `v0.1.0` сохранён неизменным; теги `v0.1.1` и `v0.1.2` не перезаписывались, новая desktop-база для end-to-end проверки — `v0.1.3`.
...
- Локально собраны подписанные NSIS и `.sig` для версий `0.1.1` и `0.1.2`; их копии и контрольные суммы находятся только в игнорируемом `src-tauri/target/updater-test/`. Сборку `0.1.3` готовит GitHub Actions после исправления теста календаря, зависевшего от часового пояса runner.
