...
- Версия текущего релиз-кандидата: `0.2.0`. Опубликованные теги `v0.1.0`–`v0.1.3` сохраняются неизменными; `v0.2.0` создаётся только после ручной проверки установщика.
...
- Локально собраны подписанные NSIS и `.sig` для версий `0.1.1` и `0.1.2`; их копии и контрольные суммы находятся только в игнорируемом `src-tauri/target/updater-test/`. Локальный `0.2.0` без updater-подписи предназначен для ручной проверки установки; подписанные updater-артефакты после неё создаёт GitHub Actions.
...
7. Вручную запустить workflow `Подготовить desktop-релиз` из проверенного релизного коммита, указав совпадающий с файлами проекта тег вида `v0.2.0` и русское название. Workflow сам создаёт тег и draft release; несовпадающие версии отклоняются до сборки.
