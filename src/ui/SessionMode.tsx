...
export function SessionMode({ campaign, isSaving, onAddEvent, onComplete, onOpenEntity, onStart, onUpdateContext }: SessionModeProps) {
  ...
  const participantChoices = activeEntities.filter((entity) => entity.id !== sceneId && entity.type !== 'scene')
  if (!view) return (
  ...
  return (
    <section className="session-section session-active" aria-labelledby="session-heading">
      <div className="session-heading"><div><p className="overline">Runtime Layer · {view.session.name}</p><h2 id="session-heading">{ru.sessionModeTitle}</h2><p>{ru.sessionModeHint}</p></div><span>{ru.worldTime}: {new Date(campaign.worldTime).toLocaleString('ru-RU')}</span></div>
      {localError && <p className="form-inline-error" role="alert">{localError}</p>}
      <div className="session-grid">
        ...
        <aside className="session-insights-panel">
          ...
          <div><h3>{ru.sessionTimeline}</h3>{view.timeline.length ? <ol className="session-timeline">{view.timeline.map((event) => { const description = describeCampaignEvent(event); return <li key={event.id}><time>{new Date(event.occurredAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</time><span><strong>{description.title}</strong>{description.detail}</span></li> })}</ol> : <p>{ru.noSessionTimeline}</p>}</div>
          <label htmlFor="session-summary">{ru.sessionSummary}</label><textarea id="session-summary" onChange={(event) => setSummary(event.target.value)} rows={4} value={summary} />
